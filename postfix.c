...
void charCat(char* c)
{
   if (postfixExp == NULL) //postfixExp가 NULL 일경우
   ...
   for(i = 0; i < length; i++) //postfixexp의 길이만큼 반복
   {
      ...
      if(getToken(symbol) == operand) { //symbol이 1이라면
         ...
         switch(getToken(symbol)) {
             case 5: evalPush(opr1 + opr2); break; //opr1과 opr2를 더한 후 evalPush함수 실행
             case 4: evalPush(opr1 - opr2); break; //opr1와 opr2를 뺀 후 evalPush함수 실행
             case 7: evalPush(opr1 * opr2); break; //opr1과 opr2를 곱한 후 evalPush함수 실행
             case 6: evalPush(opr1 / opr2); break; //op1과 opr2를 나눈 후 evalPush함수 실행
